...
export default function ExamRunner() {
  ...
  function isExamRunning(roomInfo) {
    if (!roomInfo) return true;
    const now = new Date();
    const startAt = parseDateTime(roomInfo.startAt);
    const endAt = parseDateTime(roomInfo.endAt);
  ...
  async function uploadAiFrame() {
    ...
    if (!isExamRunning(roomRef.current)) {
      if (aiTimerRef.current) clearInterval(aiTimerRef.current);
      setMsg("考试时间已结束或未开始，已停止检测与上传");
      return;
    }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (videoTrack) {
          videoTrack.onended = () => {
            setMsg("摄像头已关闭，已停止异常检测，请重新开启摄像头");
            if (aiTimerRef.current) clearInterval(aiTimerRef.current);
        ...
        client.onConnect = () => {
          ...
          if (!isExamRunning(roomRes.data)) {
            setMsg("当前不在考试时间，暂不启动异常检测");
            return;
          }
          setMsg("已进入考试房间，正在向监考老师实时推流（AI检测进行中）...");
          aiTimerRef.current = setInterval(uploadAiFrame, 50);
        };
