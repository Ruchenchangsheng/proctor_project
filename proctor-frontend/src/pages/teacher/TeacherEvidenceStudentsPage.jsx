...
export default function TeacherEvidenceStudentsPage() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            api.get(`/teacher/rooms/${examRoomId}/students`),
        ])
            .then(([evidenceResp, rosterResp]) => {
    ...
    const byStudent = useMemo(() => {
        const map = new Map();
        (students || []).forEach((s) => {
            const sid = String(s.studentId || "0");
            map.set(sid, { studentId: s.studentId, studentName: s.studentName || `学生#${sid}`, count: 0 });
        });
        (items || []).forEach((it) => {
        ...
        return Array.from(map.values()).sort((a, b) => Number(a.studentId || 0) - Number(b.studentId || 0));
    }, [items, students]);
