...
public class FaceClient {
    ...
    private org.springframework.http.ResponseEntity<Map> postEmbedWithRetry(HttpEntity<?> req, int payloadBytes) {
        ResourceAccessException last = null;
        for (int i = 1; i <= MAX_RETRY; i++) {
            try {
                return rt.postForEntity(base + "/embed", req, Map.class);
            } catch (ResourceAccessException ex) {
                last = ex;
                log.warn("调用人脸服务网络异常(第{}/{}次): base={}, bytes={}, err={}", i, MAX_RETRY, base, payloadBytes, ex.getMessage());
                if (i < MAX_RETRY) {
                    try {
                        Thread.sleep(150L * i);
        ...
        throw last == null ? new ResourceAccessException("face service unreachable") : last;
    }
    ...
    public FaceInfo extract(String mime, byte[] bytes) {
        try {
            ...
            var resp = postEmbedWithRetry(new HttpEntity<>(body, h), bytes == null ? 0 : bytes.length);
